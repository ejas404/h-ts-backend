...
export const checkOut = asyncHandler(async (req: any, res : Response) => {
    ...
    if(checkOutFor === 'cart'){
        const enrollCart = await enrollCartItems(user_id,amount)
        if(!enrollCart) throw new Error('failed to enroll cart items try later')
    }

    if(checkOutFor === 'single'){
        const enrollCourse = await enrollSingleCourse(user_id, course_id,amount)
        if(!enrollCourse) throw new Error('failed to enroll course try later')
    }
